...
export class TrainsComponent implements OnInit {
  ...
  loadTrains() {
    ...
    this.api.getDepartures().subscribe({
      next: data => {
        ...
        found.forEach(t => this.loadPopularity(t.id));
      },
      error: () => {
  ...
  loadPopularity(trainId: number) {
    this.api.getTrainDetails(trainId).subscribe({
      next: (data: any) => {
        forkJoin(data.vagons.map((v: any) => this.api.getVagonDetails(v.id))).subscribe({
          next: (results: unknown) => {
            const vagonResults = results as any[];
            let booked = 0, total = 0;
            vagonResults.forEach(vd => {
